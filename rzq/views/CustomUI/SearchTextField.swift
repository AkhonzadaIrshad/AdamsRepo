...
open class SearchTextField: UITextField {
    ...
    open var theme = SearchTextFieldTheme.lightTheme() {
        didSet {
           // tableView?.register(FilterSearchCell.self, forCellReuseIdentifier: "filtersearchcell")
            let nib = UINib.init(nibName: "FilterSearchCell", bundle: nil)
            tableView?.register(nib, forCellReuseIdentifier: "filtersearchcell")
...
//        return cell!
        ...
        cell.lblTitle.text = filteredResults[(indexPath as NSIndexPath).row].title
        cell.lblDesc.text = filteredResults[(indexPath as NSIndexPath).row].subtitle
