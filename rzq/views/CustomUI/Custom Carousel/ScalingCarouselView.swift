...
//  rzq
//
//  Created by Said Elmansour on 2021-03-04.
...
open class ScalingCarouselView: UICollectionView {
    ...
    @IBInspectable public var inset: CGFloat = 44.0 {
        didSet {
            /*
    ...
    override open func scrollToItem(at indexPath: IndexPath, at scrollPosition: UICollectionView.ScrollPosition, animated: Bool) {
        ...
        var origin = (CGFloat(indexPath.item) * (frame.size.width - (inset * 3)))
        var rect = CGRect(x: origin, y: 0, width: frame.size.width - (inset * 3), height: frame.height)
        if scrollDirection == .vertical {
            origin = (CGFloat(indexPath.item) * (frame.size.height - (inset * 3)))
            rect = CGRect(x: 0, y: origin, width: frame.width, height: frame.size.height - (inset * 3))
        }
...
fileprivate extension PrivateAPI {
    ...
    func addInvisibleScrollView(to superview: UIView?) {
        ...
        */
        ...
        invisibleScrollView.backgroundColor = .clear
        /*
    ...
    func configureLayout() {
        ...
        */
        if scrollDirection == .horizontal {
            invisibleWidthOrHeightConstraint = invisibleScrollView.widthAnchor.constraint(equalTo: widthAnchor, constant: -(3 * inset))
            invisibleLeftOrTopConstraint =  invisibleScrollView.leftAnchor.constraint(equalTo: leftAnchor, constant: inset)
        } else  {
            invisibleWidthOrHeightConstraint = invisibleScrollView.heightAnchor.constraint(equalTo: heightAnchor, constant: -(3 * inset))
