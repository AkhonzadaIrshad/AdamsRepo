...
class TenderStep1: BaseVC, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, LabasLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        ApiService.getAllTenders(name: "") { (response) in
            self.items.removeAll()
            self.items.append(contentsOf: response.data ?? [TenderData]())
...
//                                    self.longitude = 47.936161
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let orderModel = TNDROrder()
        orderModel.service = self.items[indexPath.row]
        
        
        if let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "TenderStep2") as? TenderStep2
        {
...
extension TenderStep1 : GMSMapViewDelegate {
