...
class TakeServiceOrderVC: BaseVC, AVAudioPlayerDelegate {
    
    @IBOutlet weak var mapView: UIView!
    
    @IBOutlet weak var lblDistance: MyUILabel!

    @IBOutlet weak var choosenService: MyUILabel!
    ...
    override func viewDidLoad() {
        ...
        self.lblDistance.text = self.getDistance()
        
        self.choosenService.text = self.serviceName ?? ""
        ...
        let dropOffLatLng = CLLocation(latitude: self.toLatitude ?? 0.0, longitude: self.toLatitude ?? 0.0)
    ...
    func getDistance() -> String {
        let driverLatLng = CLLocation(latitude: self.toLatitude ?? 0.0, longitude: self.toLongitude ?? 0.0)
        let dropOffLatLng = CLLocation(latitude: self.latitude ?? 0.0, longitude: self.longitude ?? 0.0)
        let distanceInMeters = dropOffLatLng.distance(from: driverLatLng)
        let distanceInKM = distanceInMeters / 1000.0
                        ...
                        if (routes.count > 0) {
                            ...
                            OperationQueue.main.addOperation({
                                for route in routes
                                {
                                    ...
                                    let pickUpPosition = CLLocationCoordinate2D(latitude: self.latitude ?? 0, longitude: self.longitude ?? 0)
...
extension TakeServiceOrderVC : GMSMapViewDelegate {
